Assignment HW7_3
 ...
 Answer the following questions:   
    * How does the $x$ dimension and $y$ dimension effect the clustering?  
    * How does the choice of neighborhood effect the clustering?  
    * How does the learning rate $\alpha (s)$ effect the clustering?    
    * Does scaling, normalization or leaving the data unscaled effect the clustering? 




So here is the R blogger script:
...
if (small_areas){
  data_raw <- read.csv("/Users/mohsennabian/Downloads/2014-01-SOM-Example-code_release/census_data/AllThemesTablesSA.csv")  
  ireland_map <- readShapePoly('/Users/mohsennabian/Downloads/2014-01-SOM-Example-code_release/boundary_files/Census2011_Small_Areas_generalised20m.shp')
  ...
  data_raw <- read.csv("/Users/mohsennabian/Downloads/2014-01-SOM-Example-code_release/census_data/AllThemesTablesED.csv")  
  names(data_raw)[1] <- "GEOGID" 
  ireland_map <- readShapePoly('/Users/mohsennabian/Downloads/2014-01-SOM-Example-code_release/boundary_files/Census2011_Electoral_Divisions_generalised20m.shp')
...
source("/Users/mohsennabian/Downloads/2014-01-SOM-Example-code_release/convertCSOdata.R")
...
source('/Users/mohsennabian/Downloads/2014-01-SOM-Example-code_release/coolBlueHotRed.R')
...
source('/Users/mohsennabian/Downloads/2014-01-SOM-Example-code_release/plotHeatMap.R')
...
#######################################
 ...
 Q1 )  How does the $x$ dimension and $y$ dimension effect the clustering?  
 
 
Here, instead of 20 * 20, I chose 10*40 .
Lets see how the rsult will change:
```{r}
 # ------------------- SOM TRAINING ---------------------------
...
if (small_areas){
  # larger grid for the small areas example (more samples)
  som_grid <- somgrid(xdim = 10, ydim=40, topo="hexagonal")  
} else {
  som_grid <- somgrid(xdim = 10, ydim=10, topo="hexagonal")  
}
# Train the SOM model!
...
As we see, since we have chosen different meshing and geometry, we end up 
different results. 
#####################################################

Q2) * How does the choice of neighborhood effect the clustering?  

We change the neighborhood from "circular" to "square".
...
By changing the neighborhood while keeping the meshing the same, we will ended up slightly different result. 
Moreover, the transient computation is highly different. Which demonstrates the importance of neghbouhood specification. 

#######################################


Q3)  * Does scaling, normalization or leaving the data unscaled effect the clustering?


The data was already scaled. So here we will use the same code without scaling and trying to see whether we end up different result. 
```{r}
...
By visualizing the result, we see that the result are different.
The reason is that the 4 attributes that we are considering are not in the same scale. They are inherently different measures. Secondly, some data points have attributes with a very large value. These points will dominate the algorithm and leads to decrease the accuracy.

In conclusion, whenever we are dealing with distance, we might 
primarily consider scaling.
